  ...
  generatePdf(sale: Sale): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      for (const it of sale.items || []) {
        if (it.imeis) totalImeis += it.imeis.length;
      }
      ...
      if (parseFloat(sale.discountTotal) > 0) {
        right(
          'Discount: -IDR ',
          parseFloat(sale.discountTotal).toLocaleString('id-ID'),
        );
      }
      ...
      for (const pay of sale.payments || []) {
        right(
          `Payment (${pay.method}):  IDR `,
          parseFloat(String(pay.amount)).toLocaleString('id-ID'),
        );
      }

      line();

      // Smartphone Warranty Policy Section
      doc
        .fontSize(6.5)
        .font('Helvetica-Bold')

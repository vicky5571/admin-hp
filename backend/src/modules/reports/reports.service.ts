...
export class ReportsService {
  ...
  // ── Sales Heatmap (Hour of Day x Day of Week) ─────────────────────
  async salesHeatmap(query: DateRangeQueryDto) {
    const params: string[] = [];
    let whereClause = `WHERE s.status IN ('COMPLETED', 'PARTIALLY_REFUNDED')`;
  ...
  async stockOnHandCsv(query: StockOnHandQueryDto): Promise<string> {
    ...
    return this.buildCsv(
      ['id', 'sku', 'name', 'product_type', 'brand', 'on_hand_qty', 'reserved_qty', 'min_stock_alert', 'cost_price', 'srp', 'stock_value'],
      data,
    );
  }
